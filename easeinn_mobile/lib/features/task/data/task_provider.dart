...
class TaskItem {
  ...
  factory TaskItem.fromJson(Map<String, dynamic> json) {
    return TaskItem(
      ...
      assignedToName: (json['assignedTo'] is Map) ? (json['assignedTo']['name'] ?? '') : '',
      roomNumber: (json['room'] is Map) ? (json['room']['roomNumber'] ?? '') : '',
...
class TaskState {
  ...
  TaskState copyWith({List<TaskItem>? tasks, bool? isLoading, String? error, int? total}) {
    return TaskState(
...
class TaskNotifier extends Notifier<TaskState> {
  ...
  Future<void> fetchTasks(String propertyId, {String? status, String? type}) async {
    state = state.copyWith(isLoading: true, error: null);
    try {
      final response = await _api.get('/properties/$propertyId/tasks', queryParameters: {
      ...
      final tasks = (data['tasks'] as List)
          ...
          .toList();
      state = state.copyWith(tasks: tasks, isLoading: false, total: data['total'] ?? tasks.length);
  ...
  Future<void> fetchMyTasks({String? status}) async {
    state = state.copyWith(isLoading: true, error: null);
    try {
      final response = await _api.get('/tasks/my', queryParameters: {
        if (status != null) 'status': status,
      });
      final data = response.data['data'];
      final tasks = (data['tasks'] as List)
      ...
      for (var t in tasks) {
        print('Task: \${t.title} | Assigned To: \${t.assignedToName}');
      }
