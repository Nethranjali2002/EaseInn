...
final webRouterProvider = Provider<GoRouter>((ref) {
  final router = GoRouter(
    ...
    routes: [
      GoRoute(
        path: '/web/login',
        builder: (c, s) => const WebLoginScreen(),
      ),
      ...
      ShellRoute(
        ...
        routes: [
          GoRoute(path: '/web', redirect: (_, __) => '/web/dashboard'),
          GoRoute(path: '/web/dashboard', builder: (c, s) => const WebDashboardScreen()),
          GoRoute(path: '/web/properties', builder: (c, s) => const WebPropertiesScreen()),
          GoRoute(path: '/web/bookings', builder: (c, s) => const WebBookingsScreen()),
          GoRoute(path: '/web/rooms', builder: (c, s) => const WebRoomsScreen()),
          GoRoute(path: '/web/tasks', builder: (c, s) => const WebTasksScreen()),
          GoRoute(path: '/web/payments', builder: (c, s) => const WebPaymentsScreen()),
          GoRoute(path: '/web/users', builder: (c, s) => const WebUsersScreen()),
          GoRoute(path: '/web/profile', builder: (c, s) => const WebProfileScreen()),
          GoRoute(path: '/web/notifications', builder: (c, s) => const WebNotificationsScreen()),
          GoRoute(path: '/web/calendar', builder: (c, s) => const WebCalendarScreen()),

          GoRoute(path: '/web/feedback', builder: (c, s) => const WebFeedbackScreen()),
          GoRoute(path: '/web/audit-log', builder: (c, s) => const WebAuditLogScreen()),
        ],
      ),
    ],
...
class _WebAppState extends ConsumerState<WebApp> {
  ...
  void initState() {
    super.initState();
    ref.read(authProvider.notifier).tryAutoLogin().timeout(const Duration(seconds: 3)).then((_) {
  ...
  Widget build(BuildContext context) {
    if (!_initialized) {
      return const MaterialApp(
        ...
        home: Scaffold(body: Center(child: CircularProgressIndicator(color: Color(0xFF1B5E20)))),
      );
    }
