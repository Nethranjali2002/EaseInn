...
class RoomState {
  ...
  RoomState copyWith({List<Room>? rooms, bool? isLoading, String? error, int? total}) {
    return RoomState(
...
class RoomNotifier extends Notifier<RoomState> {
  ...
  Future<void> fetchRooms(String propertyId, {String? status, String? roomType}) async {
    state = state.copyWith(isLoading: true, error: null);
    try {
      final response = await _api.get('/properties/$propertyId/rooms', queryParameters: {
      ...
      final rooms = (data['rooms'] as List)
          ...
          .toList();
      state = state.copyWith(rooms: rooms, isLoading: false, total: data['total'] ?? rooms.length);
  ...
  Future<bool> createRoom(String propertyId, Map<String, dynamic> data) async {
    state = state.copyWith(isLoading: true, error: null);
    try {
      final response = await _api.post('/properties/$propertyId/rooms', data: data);
